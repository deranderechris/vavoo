# -*- coding: utf-8 -*-
import os, sys, json, time, requests, requests.adapters, requests.cookies, xbmcgui, xbmc
from datetime import datetime, timedelta
from vavoo.epg.lib import xml_structure, channel_selector, mapper, filesplit
...
days_to_grab = int(utils.addon.getSetting("epg_grab_days"))+1
...
magentaDE_chlist_selected = os.path.join(temppath, 'chlist_magentaDE_selected.json')
...
def download_multithread(thread_temppath, download_threads, enable_multithread):
    ...
    if filesplit.split_chlist_selected(thread_temppath, magentaDE_chlist_selected, splitname, download_threads, enable_multithread):
        ...
        xbmc.log('%s Items to download ...' % items_to_download, xbmc.LOGINFO)
        pDialog = xbmcgui.DialogProgressBG()
        pDialog.create('{} EPG Downloader'.format(provider), '{} Items to download ...'.format(items_to_download))
        jobs = []
        for thread in range(0, int(needed_threads)):
            p = Thread(target=download_thread, args=('{}_{}.json'.format(splitname, int(thread)), multi, list, starttime, endtime, ))
            jobs.append(p)
            p.start()
        for j in jobs:
            while j.is_alive():
                ...
                pDialog.update(int(percent_completed), '{}/{} Items '.format(items, items_to_download), '{}% left. {}'.format(int(percent_remain), provider))
                if int(items) == int(items_to_download):
                    pDialog.update(int(percent_completed), '{} Download completed! '.format(provider))
                    xbmc.log('Download completed!', xbmc.LOGINFO)
                    pDialog.close()
                    break
            j.join()
...
def download_thread(magentaDE_chlist_selected, multi, list, starttime, endtime):
    ...
    for user_item in selected_list['channellist']:
        ...
        if not multi:
            ...
            percent_remain = int(100) - int(items) * int(100) / int(items_to_download)
            percent_completed = int(100) * int(items) / int(items_to_download)
            pDialog.update(int(percent_completed), '{} '.format(channel_name), '{}% left. {}'.format(int(percent_remain), provider))
            if int(items) == int(items_to_download):
                pDialog.update(int(percent_completed), '{} Download completed!'.format(provider))
                break
    if not multi:
...
def create_xml_broadcast(enable_rating_mapper, thread_temppath, download_threads, enable_multithread):
    download_multithread(thread_temppath, download_threads, enable_multithread)
